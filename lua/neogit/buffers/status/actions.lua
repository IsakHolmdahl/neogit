  ...
  return a.void(function()
    ...
    if stagable and section then
      ...
      if selection.item and selection.item.mode == "UU" then
        if
          (config.check_integration("diffview") or config.check_integration("codediff"))
          and git.merge.is_conflicted(selection.item.escaped_path)
        then
          local diff_integration = config.check_integration("codediff")
              and require("neogit.integrations.codediff")
            or require("neogit.integrations.diffview")
          diff_integration.open("conflict", selection.item.name, {
            on_close = {
    ...
    elseif section then
      ...
      elseif section.options.section == "unstaged" then
        if git.status.any_unmerged() then
          if config.check_integration("diffview") or config.check_integration("codediff") then
            local diff_integration = config.check_integration("codediff")
                and require("neogit.integrations.codediff")
              or require("neogit.integrations.diffview")
            diff_integration.open("conflict", nil, {
              on_close = {
                handle = self.buffer.handle,
                fn = function()
